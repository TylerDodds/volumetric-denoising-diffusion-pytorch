...
class GaussianDiffusion3D(Module):
    def __init__(
        self,
        model,
        *,
        ...
        assert isinstance(image_size, (tuple, list)) and len(image_size) == 3, 'image size must be a integer or a tuple/list of three integers'
    ...
    def forward(self, img, *args, **kwargs):
        ...
        assert h == img_size[0] and w == img_size[1] and d == img_size[2], f'height and width and depth of image must be {img_size}'
...
class Trainer3D:
    def __init__(
        ...
        *,
        ...
        assert (train_batch_size * gradient_accumulate_every) >= min_effective_batch_size, f'your effective batch size (train_batch_size x gradient_accumulate_every) should be at least {min_effective_batch_size} or above'
        ...
        assert len(self.ds) >= 10, 'you should have at least 10 samples in your dataset. At least 10k is recommended.'
        ...
        # Note that we don't have a 3D FID-score computation at this point.
            ...
            while self.step < self.train_num_steps:
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and divisible_by(self.step, self.save_and_sample_every):
                        ...
                        image_index = 0
                        for image_batch in all_images_list:
                            for image in image_batch:
                                self.saver.save(self.results_folder, f'sample-{milestone}-{image_index}', image.squeeze().cpu())
                                image_index += 1

                        if self.save_best_and_latest_only:
                            #NB Since we don't have FID available in this case, we will rely only on total_loss for now.
